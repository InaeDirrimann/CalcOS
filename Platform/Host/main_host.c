...
/* Strip C0 control bytes (incl. ESC) so piped input cannot inject terminal
 * control sequences into the echoed output. UTF-8 continuation bytes
 * (0x80+) are preserved. */
static void sanitize_echo(char* s) {
    char* w = s;
